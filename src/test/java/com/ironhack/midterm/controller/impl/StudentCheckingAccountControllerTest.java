...
//        accountRepository.deleteAll();
    ...
    @Test
    void getStudentCheckingAccounts() throws Exception{
        MvcResult result = mockMvc.perform(get("/student-checking-accounts")).andDo(print()).andExpect(status().isOk()).andReturn();
        assertTrue(result.getResponse().getContentAsString().contains("23232"));
        assertFalse(result.getResponse().getContentAsString().contains("55555"));
    }

    @Test
    void store() throws Exception{
        StudentCheckingAccount studentCheckingAccount1 = new StudentCheckingAccount(new Money(new BigDecimal(2000)), "6543", accountHolder5, List.of());
        String body = objectMapper.writeValueAsString(studentCheckingAccount1);
        MvcResult result = mockMvc.perform(
                post("/student-checking-accounts/new")
                        .content(body)
                        .contentType(MediaType.APPLICATION_JSON)
        ).andDo(print()).andExpect(status().isCreated()).andReturn();
        assertTrue(result.getResponse().getContentAsString().contains("6543"));
    }
