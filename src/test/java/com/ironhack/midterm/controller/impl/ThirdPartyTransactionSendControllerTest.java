...
@SpringBootTest
class ThirdPartyTransactionSendControllerTest {
    ...
    @Autowired
    private IThirdPartyTransactionSendController thirdPartyTransactionController;
    ...
    private ThirdPartyTransactionSend thirdPartyTransactionSend1;
    private ThirdPartyTransactionSend thirdPartyTransactionSend2;
    ...
    void setUp() {
        ...
        thirdPartyTransactionSend1 = new ThirdPartyTransactionSend(TransactionType.TRANSFER, new Money(new BigDecimal(300)), thirdParty1, creditCardAccount1);
        thirdPartyTransactionSend1.setTimeStamp(LocalDateTime.of(2021,9,20,12,0));
        thirdPartyTransactionSend2 = new ThirdPartyTransactionSend(TransactionType.TRANSFER, new Money(new BigDecimal(400)), thirdParty2, studentCheckingAccount1);
        thirdPartyTransactionSend2.setTimeStamp(LocalDateTime.of(2021,8,27,12,0));
        thirdPartyTransactionSendRepository.saveAll(List.of(thirdPartyTransactionSend1, thirdPartyTransactionSend2));
...
//        accountRepository.deleteAll();
    ...
    @Test
    void getThirdPartyTransactions() throws Exception{
        MvcResult result = mockMvc.perform(get("/third-party-send-transactions")).andDo(print()).andExpect(status().isOk()).andReturn();
    ...
    void getById() throws Exception{
        MvcResult result = mockMvc.perform(
                get("/third-party-send-transactions/"+ thirdPartyTransactionSend1.getId())
        ).andDo(print()).andExpect(status().isOk()).andReturn();
        assertTrue(result.getResponse().getContentAsString().contains("200"));
        assertFalse(result.getResponse().getContentAsString().contains("400"));
    }

    @Test
    void getByHashedKey() throws Exception{
        MvcResult result = mockMvc.perform(
                get("/third-party/send-money/"+thirdParty1.getHashedKey())
        ).andDo(print()).andExpect(status().isOk()).andReturn();
        assertTrue(result.getResponse().getContentAsString().contains("200"));
        assertFalse(result.getResponse().getContentAsString().contains("400"));
    }
    ...
    void transfer() throws Exception{
        ...
        ThirdPartyTransactionSendDTO thirdPartyTransaction1 = new ThirdPartyTransactionSendDTO(TransactionType.TRANSFER, new Money(new BigDecimal(50)), checkingAccount1.getId(), checkingAccount1.getSecretKey());
        String body = objectMapper.writeValueAsString(thirdPartyTransaction1);
        MvcResult result = mockMvc.perform(
                post("/third-party/send-money/"+thirdParty1.getHashedKey())
                        .content(body)
        ...
        Account checkAccount2= accountRepository.findById(thirdPartyTransaction1.getRecipientAccountId()).get();
        BigDecimal recipientAccountBalance1 = checkAccount2.getBalance().getAmount();
        assertEquals(transfer, recipientAccountBalance1.subtract(recipientAccountBalance0));
    }
}