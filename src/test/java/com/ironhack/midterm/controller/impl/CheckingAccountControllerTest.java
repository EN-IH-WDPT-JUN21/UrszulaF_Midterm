...
//        accountRepository.deleteAll();
    ...
    @Test
    void getCheckingAccounts() throws Exception{
        MvcResult result = mockMvc.perform(get("/checking-accounts")).andDo(print()).andExpect(status().isOk()).andReturn();
    ...
    void store() throws Exception{
        CheckingAccount checkingAccount1 = new CheckingAccount(new Money(new BigDecimal(8000)), "43210", accountHolder2, List.of(accountHolder4));
        String body = objectMapper.writeValueAsString(checkingAccount1);
        MvcResult result = mockMvc.perform(
                post("/checking-accounts/new")
                        .content(body)
                        .contentType(MediaType.APPLICATION_JSON)
        ).andDo(print()).andExpect(status().isCreated()).andReturn();
        assertTrue(result.getResponse().getContentAsString().contains("43210"));
    }
}